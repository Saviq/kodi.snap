...
DEPENDENCY_ROOT = (
    "https://raw.githubusercontent.com/{owner}/{repo}/{branch}/depends/common/{dep}/"
)
try:
    ADDONS_BRANCH = sys.argv[1]
except IndexError as ex:
    raise SystemExit("Usage: update_addons.py <branch>") from ex
...
assert isinstance(source_craft, dict), "Source craft must be a dictionary"
parts: dict[str, Any] = {
    "kodi": {
...
def retrieve_dep(filename, vars, method=requests.get):
    if (
        r := method((DEPENDENCY_ROOT + filename).format(**vars))
...
with TemporaryDirectory() as tmp:
    subprocess.run(
        ("git", "clone", ADDONS_REPO, "--branch", ADDONS_BRANCH, "--depth=1", tmp),
        check=True,
    )
    ...
    for platforms in sorted(Path(tmp).glob("*/platforms.txt")):
        with (
            open(platforms) as f,
            open(platforms.parent / (platforms.parent.name + ".txt")) as f2,
        ):
            supported_platforms = set(next(f).strip().split())
            addon, repourl, branch = next(f2).strip().split()

        if addon in DENY_LIST:
            continue

        if supported_platforms.intersection({"all", "linux"}) or all(
            platform.startswith("!") for platform in supported_platforms
        ):
            ...
            for dep in after:
                ...
                if flags := retrieve_dep("flags.txt", locals()):
                    parts[dep]["cmake-parameters"] = (
                        parts[dep]["cmake-parameters"] + flags.split()
                    )
...
parts["gpu-2404"] = {"after": ["kodi", *(addon for addon in parts)]}

craft = hiyapyco.load(
    hiyapyco.dump({"parts": parts}), SOURCE, method=hiyapyco.METHOD_MERGE
)
